...
import type { PermissionRuleValue, PermissionUpdate } from "@betterc0de/schema"
...
function remainingInputJson(input: Record<string, unknown>): string | undefined {
...
export function approvalInputView(approval: PendingApproval): ApprovalInputView {
...
export function alwaysAllowRules(
  ...
  const suggested = approval.suggestions?.find(
    (update) =>
  ...
  const ruleContent = alwaysAllowRuleContent(approval, toolName)
...
export function buildAlwaysAllowUpdate(
  ...
  const rules = alwaysAllowRules(approval)
...
export function alwaysAllowRuleContent(
  ...
  const filePath = view.filePath?.trim()

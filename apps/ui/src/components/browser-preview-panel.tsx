...
import { useBrowserContextStore, MAX_BROWSER_ELEMENTS } from "@/lib/browser-context-store"
...
import {
  DEVICE_PRESETS,
} from "./browser-preview/constants"
...
export function BrowserPreviewPanel({
  ...
  const targetThreadId = useChatStore(state => state.activeThreadId)
  const [selectionMode, setSelectionMode] = useState(isElectron)
  const [initialUrl] = useState(() => {
    try { return previewUrl(localStorage.getItem(`betterc0de-preview-url:${projectPath ?? ""}`) || "") || "http://localhost:3000" } catch { return "http://localhost:3000" }
  })

  const [url, setUrl] = useState(initialUrl)
  const [inputUrl, setInputUrl] = useState(initialUrl)
  const [currentUrl, setCurrentUrl] = useState(initialUrl)
  const [pageHistory, setPageHistory] = useState({ back: false, forward: false })
  ...
  useEffect(() => {
    try { localStorage.setItem(`betterc0de-preview-url:${projectPath ?? ""}`, currentUrl) } catch { /* Storage can be unavailable. */ }
  ...
  const handleElementSelected = useCallback((el: SelectedElement) => {
    ...
    const element = normalizeBrowserElement({ ...el, url: el.url || currentUrl })
    if (!element) return
    const store = useChatStore.getState()
    const target = store.threads.find(thread => thread.id === targetThreadId)
    if (targetThreadId && !target) return
    const threadId = targetThreadId ?? store.createThread("New Chat", projectPath?.split(/[\\/]/).pop() || "Workspace", projectPath ?? undefined)
    if (!useBrowserContextStore.getState().add(threadId, element)) {
      toast.info(`You can select up to ${MAX_BROWSER_ELEMENTS} elements per message`)
    }
  ...
  const goBack = useCallback(() => {
    if (isElectron) { viewportRef.current?.goBackInPage(); return }
    if (historyIndex > 0) {
  ...
  const goForward = useCallback(() => {
    if (isElectron) { viewportRef.current?.goForwardInPage(); return }
  ...
  const sendCssToAI = useCallback(() => {
    ...
    if (!threadId) threadId = store.createThread("Visual Changes", projectPath?.split(/[\\/]/).pop() || "Workspace", projectPath ?? undefined)
    dispatchComposerDraftRestoreAfterSubmit({
      threadId,
      text: [store.getDraft(threadId), cssChangesPrompt(cssChanges, currentUrl)]
        .filter(Boolean)
        .join("\n\n"),
    })
    setInspector(withoutCssChanges)
  }, [cssChanges, currentUrl, projectPath])
  ...
  return (
    <div
      data-betterc0de-preview="browser"
      className={cn("flex flex-col bg-background", className)}
    >
      {/* Toolbar */}
      <div className="flex shrink-0 items-center gap-1 border-b border-border/40 bg-muted/20 px-2 py-1">
        <TooltipProvider>
          <Tooltip>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                size="icon-xs"
                onClick={goForward}
                disabled={isElectron ? !pageHistory.forward : historyIndex >= history.length - 1}
              >
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="ghost" size="icon-xs" onClick={refresh} aria-label="Reload preview">
                <RefreshCwIcon
        ...
        <Input
          ...
          className="h-6 flex-1 rounded-lg border-border/40 bg-sidebar px-2 font-mono text-[11px]"
          value={inputUrl}
        ...
        <Button variant="ghost" size="icon-xs" onClick={onClose} aria-label="Close browser preview">
          ...
          <div className="flex min-h-0 flex-1 items-center justify-center overflow-hidden bg-muted/10">
            ...
            >
              <PreviewViewport
                ...
                onNavigate={(nextUrl, nextHistory) => { setInputUrl(nextUrl); setCurrentUrl(nextUrl); setPageHistory(nextHistory) }}
      ...
      <div className="flex shrink-0 items-center gap-3 border-t border-border/40 bg-muted/20 px-3 py-0.5 text-[10px] text-muted-foreground">
        <span className="truncate">{selectionMode ? "Click elements to add them to chat · Esc to browse" : "Browse the page · enable Select to pick elements"}</span>
        <div className="flex-1" />
        {selectedElement && (

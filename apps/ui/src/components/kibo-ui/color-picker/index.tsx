...
export const ColorPicker = ({
  ...
  //
  // Bug history: the previous implementation did `Color.rgb(value).rgb().object()`
  // which returns `{r,g,b,a}` (0–255 range) and assigned those channels to the
  // hue / saturation / lightness setters. Channel-count happened to match so it
  // never crashed — the picker just rendered the wrong colour whenever it was
  // driven by a `value` prop. Conversion is now isolated in `deriveHslState`
  // so it can be unit-tested without a renderer.
  useEffect(() => {
    if (value === undefined || value === null) return;
    const derived = deriveHslState(value);
    if (!derived) return;
    setHue(derived.hue);
    setSaturation(derived.saturation);
    setLightness(derived.lightness);
    setAlpha(derived.alpha);
  }, [value]);

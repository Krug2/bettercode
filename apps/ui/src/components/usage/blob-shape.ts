export function blobShape(radius: number, phase: number, stretch: number, direction: number, ripple = 0): string {
  const count = 10
  const points = Array.from({ length: count }, (_, index) => {
    const angle = index / count * Math.PI * 2
    const organic = 1 + 0.03 * Math.sin(angle * 3 + phase) + 0.02 * Math.cos(angle * 2 - phase)
    const r = radius * organic * (1 + stretch * Math.cos(2 * (angle - direction)) + ripple * Math.sin(3 * (angle - direction) + phase))

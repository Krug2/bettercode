...
export function useBlobScene(nodes: BlobNode[], visible: BlobNode[], positions: Record<string, Point>, camera: UsageLayout["camera"]) {
  ...
  const paint = useCallback(() => {
    ...
    for (const [id, body] of physics.bodies) {
      ...
      if (button) {
        ...
        button.style.setProperty("--blob-tilt", `${(body.slosh.x / body.node.radius * 12).toFixed(3)}deg`)
      }
      if (body.visible) {
        const stretch = Math.hypot(body.strain.x, body.strain.y)
        shapes.current.get(id)?.setAttribute("d", blobShape(body.node.radius * 0.95, body.phase, stretch, Math.atan2(body.strain.y, body.strain.x) / 2))
      }
  ...
  const wake = useCallback(() => {
    ...
    const tick = (now: number) => {
      ...
      previous = now
      const energy = physics.step(elapsed, motionPreference.current)
      ...
      const rate = motionPreference.current ? 1 : 1 - Math.pow(0.82, elapsed / 16.667)
      ...
      if ((energy > 0.025 || cameraEnergy > 0.1) && ++steps < 600) {
        ...
        frame.current = 0
        if (!physics.dragging) {
          const saved = physics.takePositions()
          if (Object.keys(saved).length) useUsageGraphStore.getState().move(saved)
        }
      }
    }
    frame.current = requestAnimationFrame(tick)
  ...
  useEffect(() => () => {
    ...
    frame.current = 0
    const saved = physics.dragging ? physics.releaseBranch() : physics.takePositions()
    if (Object.keys(saved).length) useUsageGraphStore.getState().move(saved)
  }, [physics])

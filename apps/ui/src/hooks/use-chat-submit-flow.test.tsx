...
import { buildBetterC0deDefaultAgentContext, providerSkillSlashPrompt, runRegisteredSlashCommand } from "@/lib/slash-command-runtime"
...
describe("chat submission ownership", () => {
  ...
  it.each(["/goal pause", "/goal status", "/goal clear"])("sends %s immediately to its own chat without resolving or starting a provider", async text => {
    useChatStore.getState().appendStreamDelta("a", "Working")
    const queued = useMessageQueueStore.getState().enqueue("a", { text: "Later", files: [], browserElements: [] })
    await handler()({ text, files: [], threadId: "a" })
    expect(sendGoalControl).toHaveBeenCalledExactlyOnceWith("a", text, "gpt-6-astra")
    expect(resolveProviderTarget).not.toHaveBeenCalled()
    expect(sendChatMessage).not.toHaveBeenCalled()
    expect(useMessageQueueStore.getState().messages.map(entry => entry.id)).toEqual([queued.id])
  ...
  it.each(["claude", "grok_cli", "codex"])("routes goals through the selected %s provider without adding a normal chat message", async providerKind => {
    const provider: UiProvider = { ...codex, id: providerKind, providerKind, providerInstanceId: `${providerKind}-instance` }
    usePreferencesStore.setState({ selectedProviderId: provider.id })
    for (const text of ["/goal Fix My UI", "/goal continue"]) {
      vi.mocked(resolveProviderTarget).mockResolvedValueOnce({ providerKind, providerInstanceId: provider.providerInstanceId!, openaiTransport: null })
      await handler([provider])({ text, files: [], threadId: "a" })
      expect(resolveProviderTarget).toHaveBeenLastCalledWith(provider, "gpt-6-astra")
      const request = vi.mocked(sendChatMessage).mock.calls.at(-1)!
      expect(request.slice(0, 4)).toEqual(["a", text, "gpt-6-astra", providerKind])
  ...
  it("sends edited multiline text and literal paste markers intact to their own chats", async () => {
    ...
    expect(vi.mocked(sendChatMessage).mock.calls.map(call => call.slice(0, 2))).toEqual([["a", textA], ["b", textB]])
    expect(useChatStore.getState().threads.map(thread => thread.messages[0].content)).toEqual([textA, textB])
  })

  it("retains editable pasted text when queued delivery fails and is retried", async () => {
    const text = "Edited paste\n  original indentation\n\n" + "content ".repeat(100) + "\n[Pasted ~3 lines #1]"
    useChatStore.getState().appendStreamDelta("a", "Working")
    await handler()({ text, files: [], threadId: "a" })
    ...
    const payload = { ...entry.payload, threadId: "a", queuedSubmission: { id: entry.id, createdAt: entry.createdAt, browserElements: [] } }
    ...
    expect(vi.mocked(sendChatMessage).mock.calls.map(call => call.slice(0, 2))).toEqual([["a", text], ["a", text]])
  ...
  it("dispatches a typed skill through the existing turn path and retains the user's slash text", async () => {
    vi.mocked(providerSkillSlashPrompt).mockReturnValueOnce("$seo-audit inspect this project")
    await handler()({ text: "/seo-audit inspect this project", files: [], threadId: "a" })
    expect(runRegisteredSlashCommand).not.toHaveBeenCalled()
    ...
    expect(request[14]).toMatchObject({ content: "/seo-audit inspect this project" })
    expect(useChatStore.getState().threads[0].messages[0].content).toBe("/seo-audit inspect this project")
  })

  it("sends captured element references with their owning chat and retains later picks", async () => {
    const element = { url: "https://example.com/", selector: "#buy", tagName: "button", text: "Buy", label: "Buy" }
    useBrowserContextStore.getState().add("a", element)
    const entered = Promise.withResolvers<void>()
    const context = Promise.withResolvers<string | null>()
    vi.mocked(buildBetterC0deDefaultAgentContext).mockImplementationOnce(() => { entered.resolve(); return context.promise })
    const sending = handler()({ text: "Make this smaller", files: [], threadId: "a" })
    await entered.promise
    useChatStore.getState().setActiveThread("b")
    useBrowserContextStore.getState().add("a", { ...element, selector: "#later" })
    ...
    expect(useChatStore.getState().threads[0].messages[0].attachments).toEqual(request[18])
    expect(useBrowserContextStore.getState().byThread.a.map(item => item.selector)).toEqual(["#later"])
  ...
  it("captures queued tags and attachments without consuming tags picked again for the next draft", async () => {
    const element = { url: "https://example.com/", selector: "#buy", tagName: "button", text: "Buy", label: "Buy" }
    const file = { type: "file", url: "data:text/plain;base64,SGk=", filename: "notes.txt" }
    useBrowserContextStore.getState().add("a", element)
    useChatStore.getState().appendStreamDelta("a", "Working")
    expect(await handler()({ text: "Change this next", files: [file], threadId: "a" })).toBe(true)
    const entry = useMessageQueueStore.getState().messages[0]
    ...
    useBrowserContextStore.getState().add("a", { ...element, selector: "#later" })
    useChatStore.getState().clearStreaming("a")
    await handler()({ ...entry.payload, threadId: "a", queuedSubmission: { id: entry.id, createdAt: entry.createdAt, browserElements: entry.payload.browserElements } })
    expect(vi.mocked(sendChatMessage).mock.calls[0][1]).toContain('"selector":"#buy"')
    expect(vi.mocked(sendChatMessage).mock.calls[0][1]).not.toContain("#later")
    expect(useBrowserContextStore.getState().byThread.a.map(value => value.selector)).toEqual(["#buy", "#later"])
  })

  it("reports failed queued delivery to the runner instead of dropping the pending message", async () => {
    const entry = useMessageQueueStore.getState().enqueue("a", { text: "Pending", files: [], browserElements: [] })
    const payload = { ...entry.payload, threadId: "a", queuedSubmission: { id: entry.id, createdAt: entry.createdAt, browserElements: [] } }
    vi.mocked(sendChatMessage).mockRejectedValueOnce(new Error("Connection lost"))
    await expect(handler()(payload)).rejects.toThrow("Connection lost")
    expect(useMessageQueueStore.getState().messages[0].id).toBe(entry.id)
    await handler()(payload)
    expect(useChatStore.getState().threads[0].messages.filter(message => message.id === entry.id)).toHaveLength(1)
  })
  ...
  it("queues additional messages during preparation without blocking another thread", async () => {
    ...
    await entered.promise
    expect(await handler()({ text: "Follow-up", files: [], threadId: "a" })).toBe(true)
    expect(useMessageQueueStore.getState().messages[0]).toMatchObject({ threadId: "a", payload: { text: "Follow-up" }, status: "queued" })
    await handler()({ text: "Independent", files: [], threadId: "b" })
    context.resolve(null)
    await sending
    expect(vi.mocked(sendChatMessage).mock.calls.map(call => call[0])).toEqual(["b", "a"])
    expect(useChatStore.getState().threads[0].messages.filter(m => m.role === "user")).toHaveLength(1)
    expect(await handler()({ text: "Still busy", files: [], threadId: "a" })).toBe(true)
    useChatStore.getState().clearStreaming("a")
    const entry = useMessageQueueStore.getState().messages[0]
    await handler()({ ...entry.payload, threadId: "a", queuedSubmission: { id: entry.id, createdAt: entry.createdAt, browserElements: entry.payload.browserElements } })
    expect(sendChatMessage).toHaveBeenCalledTimes(3)
    expect(vi.mocked(sendChatMessage).mock.calls[2][14]).toMatchObject({ id: entry.id, content: "Follow-up", createdAt: entry.createdAt })
  })
  ...
  it("does not erase an active stream or generate error bubbles on a busy conflict", async () => {
    vi.mocked(sendChatMessage).mockImplementationOnce(async () => {
      ...
      throw new HttpError("Thread 'a' already has active provider work.", 409, "/chat/send")
    })
    expect(await handler()({ text: "Hello", files: [], threadId: "a" })).toBe(false)
    expect(useChatStore.getState().streamingByThread.a).toMatchObject({
      isStreaming: true, activeTurnId: "running-turn", streamingText: "Existing work",
    })
    expect(useChatStore.getState().threads[0].messages.some(m => m.content.startsWith("Error:"))).toBe(false)
    expect(await handler()({ text: "Retry", files: [], threadId: "a" })).toBe(true)

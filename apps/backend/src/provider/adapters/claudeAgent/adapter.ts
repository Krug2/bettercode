import type { ModelDefinition, ProviderKind, ProviderSendTurnInput } from "../../types";
...
import { claudeAgentModels } from "./auth-flow";
...
export class ClaudeAgentAdapter extends BaseProviderAdapter {
  ...
  protected _activeSessionIds(): string[] {
    return Array.from(new Set([...this.sessions.keys(), ...this.abortFlags.keys()]));
  }

  providerKind(): ProviderKind { return "anthropic_cli"; }
  displayName(): string { return "Claude (CLI)"; }

  availableModels(): ModelDefinition[] { return claudeAgentModels(); }
  ...
  async interrupt(threadId: string): Promise<void> {
    ...
    try {
      ...
      logger.warn({ err: (err as Error).message, thread: threadId }, "Claude Agent interrupt failed");
      throw err;
    }
  ...
  async sendMessage(input: ProviderSendTurnInput): Promise<void> {
    if (this.abortFlags.has(input.thread_id)) throw new Error("Claude Agent turn is already active");
    ...
    try {
      ...
      const projectToolPolicy = await loadBetterC0deProjectToolPolicy(input.project_path);
      if (this.abortFlags.get(input.thread_id)) return;
      const canUseTool = buildCanUseTool(input.thread_id, level, (e) => this.emit(e), chatMode, {
        projectPermissionRules: projectToolPolicy.permissionRules,
      });
      const tools = filterToolsForBetterC0deProjectPolicy(getToolsForMode(chatMode), {
      ...
      const runtime = sdk.query({
        ...
        options: {
          ...
          ...(claudeCodeBinaryPath ? { pathToClaudeCodeExecutable: claudeCodeBinaryPath } : {}),
      ...
      try {
        for await (const rawMsg of runtime) {
          ...
          if (process.env.BETTERC0DE_TRACE_PROVIDER_EVENTS === "1") {
            ...
            const _r = rawMsg as { type?: string; subtype?: string; event?: { delta?: { type?: string } } };
            console.log(`[REASON-TRACE:CLAUDE-SDK-IN] type=${_r?.type ?? "-"} subtype=${_r?.subtype ?? "-"} hasEvent=${!!_r?.event} eventDeltaType=${_r?.event?.delta?.type ?? "-"} keys=${rawMsg && typeof rawMsg === "object" ? Object.keys(rawMsg).join(",") : "-"}`);
          }
      ...
      logger.error({ err: message, thread: input.thread_id }, "Claude Agent stream failed");
      throw err;
    } finally {
      this.abortFlags.delete(input.thread_id);
    }

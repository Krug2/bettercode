...
describe("CodexHomeLayout", () => {
  describe("resolveCodexHomeLayout", () => {
    ...
    it("uses the shared home for continuation and the shadow home for runtime auth", () => {
      ...
      expect(resolveCodexHomeLayout({ homePath: sharedHome, shadowHomePath: shadowHome }))
        .toMatchObject({
          mode: "authOverlay",
          sharedHome,
  ...
  describe("materializeCodexShadowHome", () => {
    it.skipIf(process.platform !== "win32")("respects case-insensitive private files and shared directories on Windows", () => {
      ...
      expect(readLink(path.join(overlay, "sessions"))).toBe(path.join(source, "SESSIONS"));
      expect(fs.readdirSync(overlay).filter(name => name.toLowerCase() === "sessions")).toHaveLength(1);
    });
    ...
    it("does not create directories or detach a cache when any destination conflicts", () => {
      ...
      fs.symlinkSync(path.join(source, "models_cache.json"), path.join(overlay, "models_cache.json"));
      expect(() => materializeCodexShadowHome(source, overlay)).toThrow(CodexShadowHomeError);
      expect(fs.readdirSync(source)).toEqual(["models_cache.json"]);
      expect(isSymlink(path.join(overlay, "models_cache.json"))).toBe(true);
      expect(fs.readFileSync(path.join(overlay, "worktrees"), "utf8")).toBe("keep this file");
    });
    ...
    it("rejects directory aliases and nesting before changing the filesystem", () => {
      ...
      for (const target of [alias, path.join(source, "overlay")]) {
        expect(() => materializeCodexShadowHome(source, target)).toThrow(/non-overlapping/);
      }
    ...
    it("reuses correct links and repairs stale links without modifying their old targets", () => {
      ...
      fs.symlinkSync(old, path.join(overlay, "sessions"), process.platform === "win32" ? "junction" : "dir");
      materializeCodexShadowHome(source, overlay);
      materializeCodexShadowHome(source, overlay);
      expect(readLink(path.join(overlay, "sessions"))).toBe(path.join(source, "sessions"));
      expect(fs.readFileSync(path.join(old, "keep.txt"), "utf8")).toBe("old session");
    });
    ...
    it("validates private auth before creating shared runtime state", () => {
      ...
      expect(() => materializeCodexShadowHome(source, overlay)).toThrow(/must be a real file/);
    ...
    it("materializes a shadow home with shared state links and private auth", () => {
      ...
      writeTextFile(path.join(sharedHome, "config.toml"), 'model = "gpt-5-codex"\n');
      writeTextFile(path.join(sharedHome, "models_cache.json"), '{"models":["shared"]}\n');
      ...
      expect(readLink(path.join(shadowHome, "sessions"))).toBe(path.join(sharedHome, "sessions"));
    ...
    it("shares non-private Codex sidecars instead of copying stale shadow files", () => {
      ...
      writeTextFile(path.join(sharedHome, "credentials.json"), '{"shared":true}\n');
    ...
    it("rejects shadow homes that point at the shared home", () => {
      ...
      expect(() =>
        resolveCodexHomeLayout({ homePath: sharedHome, shadowHomePath: sharedHome }),
    ...
    it("rejects shadow auth that points back to shared auth", () => {
      ...
      writeTextFile(path.join(sharedHome, "auth.json"), '{"shared":true}\n');
      fs.mkdirSync(shadowHome, { recursive: true });
      fs.symlinkSync(path.join(sharedHome, "auth.json"), path.join(shadowHome, "auth.json"));

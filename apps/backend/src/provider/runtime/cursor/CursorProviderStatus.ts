...
import { buildWindowsCmdArgs, requiresWindowsCmdWrapper, resolveComSpec } from "../../../security/windowsCommandLine"
import { appendBoundedProcessOutput, createBoundedProcessOutput, processOutputLimitError } from "../BoundedProcessOutput"
...
const statusCache = new Map<string, { ts: number; result: CursorProviderStatusProbe }>()
...
async function probeCursorProviderStatusUncached(input: {
  ...
  const aboutResult = await runCursorAboutCommand(command, env).catch(
    (error: unknown): CursorCommandResult & { readonly commandMissing?: boolean } => ({
      stdout: "",
  ...
  if (aboutResult.code === -1) {
    return {
      installed: true, configured: false, version: null, status: "error",
      auth: { status: "unknown" },
      message: "Cursor Agent status could not be verified. Retry the status check.",
    }
  }
...
export function parseCursorAboutOutput(result: CursorCommandResult): CursorAboutResult {
  const json = parseCursorAboutJsonPayload(result.stdout)
  const transcript = [result.stdout, result.stderr].join("\n")
  if (!json && /unknown command|unrecognized command|unexpected argument/i.test(transcript)) {
    return {
      status: "warning", version: null, auth: { status: "unknown" },
      message: "The `agent about` command is unavailable in this version of the Cursor Agent CLI.",
    }
  }
  const field = (value: unknown) => typeof value === "string" ? value.trim() : undefined
  const plain = json ? "" : stripAnsi(transcript)
  const version = (json ? field(json.cliVersion) : extractAboutField(plain, "CLI Version")) ?? null
  const email = json ? field(json.userEmail) : extractAboutField(plain, "User Email")
  const explicitlyLoggedOut = json && hasOwn(json, "userEmail") && json.userEmail == null
  if (explicitlyLoggedOut || (email !== undefined && isUnauthenticatedCursorEmail(email))) {
    return unauthenticatedCursorAbout(version)
  }
  const metadata = json ? cursorAuthMetadata(field(json.subscriptionTier)) : undefined
  if (email) {
    return { auth: { ...metadata, status: "authenticated", email }, status: "ready", version }
  }
  if (result.code !== 0) {
    return {
      auth: { status: "unknown" }, status: "warning", version,
...
export function parseCursorVersionDate(version: string | null | undefined): number | undefined {
  const date = /^(\d{4})\.(\d{2})\.(\d{2})(?:\b|-|$)/.exec(version?.trim() ?? "")
  return date ? Number(date[1]) * 10_000 + Number(date[2]) * 100 + Number(date[3]) : undefined
}

export function parseCursorCliConfigChannel(
  raw: string
): string | undefined {
  try {
    ...
    if (
      ...
      const channel = (parsed as { channel: string }).channel.trim().toLowerCase()
...
async function runCursorAboutCommand(
  ...
  const jsonResult = await runCommand(binaryPath, ["about", "--format", "json"], {
    env,
...
async function runCommand(
  ...
  if (failedStatusCommand) {
    if (process.platform === "win32" && (failedStatusCommand.exitCode !== null || failedStatusCommand.signalCode !== null)) {
      ...
      throw new Error("Cursor status process-tree cleanup remains unconfirmed after its root exited.")
    }
    if (!statusCleanupInFlight) {
      const child = failedStatusCommand
      const cleanup = terminateProviderChildProcessTree(child).then(() => {
  ...
  if (activeStatusCommand) throw new Error("Another Cursor status command is already running.")
  return new Promise((resolve, reject) => {
    const viaCmd = requiresWindowsCmdWrapper(command)
    const child = spawn(viaCmd ? resolveComSpec() : command, viaCmd ? buildWindowsCmdArgs(command, args) : [...args], {
      env: options.env,
      shell: false,
    ...
    const finish = (error?: Error, code?: number | null) => {
      ...
      else resolve({ stdout: output.stdout, stderr: output.stderr, code: code ?? -1 })
    }
    const beginTermination = (error: Error) => {
      ...
      void terminateProviderChildProcessTree(child).then(
        ...
        (cleanupError) => {
          ...
          finish(new AggregateError([error, cleanupError], `${error.message} Process-tree cleanup also failed.`))
        },
      )
    }
    const timer = setTimeout(() => {
      beginTermination(new Error(`Command timed out after ${options.timeoutMs}ms.`))
    }, options.timeoutMs)

    child.stdout?.setEncoding("utf8")
    child.stderr?.setEncoding("utf8")
    child.stdout?.on("data", (chunk) => {
      if (!settled && !terminating && !appendBoundedProcessOutput(output, "stdout", chunk)) {
        beginTermination(processOutputLimitError("Cursor status probe", output.byteCap))
      }
    })
    child.stderr?.on("data", (chunk) => {
      if (!settled && !terminating && !appendBoundedProcessOutput(output, "stderr", chunk)) {
        beginTermination(processOutputLimitError("Cursor status probe", output.byteCap))
      }
    })
...
function parseCursorAboutJsonPayload(raw: string): CursorAboutJsonPayload | undefined {
  try {
    ...
    return value && typeof value === "object" && !Array.isArray(value)
      ? value as CursorAboutJsonPayload : undefined
  } catch { return undefined }
}

function isCursorAboutJsonFormatUnsupported(result: CursorCommandResult): boolean {
...
function unauthenticatedCursorAbout(
  version: string | null
): CursorAboutResult {
  return {
    version,
...
function cursorSubscriptionLabel(subscriptionType: string | undefined): string | undefined {
  ...
  return standard.find(label => label.toLowerCase() === key) ?? toTitleCaseWords(subscriptionType)
}

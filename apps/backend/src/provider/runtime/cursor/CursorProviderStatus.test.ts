...
async function makeProbe(directoryName: string, source: string): Promise<string> {
  ...
  const binary = path.join(directory, process.platform === "win32" ? "cursor-agent.cmd" : "cursor-agent")
  await fs.writeFile(binary, process.platform === "win32"
...
describe("CursorProviderStatus", () => {
  it("probes configured shim paths containing spaces and shell operators literally", async () => {
    const binaryPath = await makeProbe("Cursor Agent & CLI", 'console.log(JSON.stringify({cliVersion:"2026.04.08",userEmail:"safe@example.com"}))')
    const result = await probeCursorProviderStatus({ binaryPath })
    expect(result.auth).toMatchObject({ status: "authenticated", email: "safe@example.com" })
  })

  it("does not reuse authentication status across different provider environments", async () => {
    const binaryPath = await makeProbe("cli", 'console.log(JSON.stringify({cliVersion:"2026.04.08",userEmail:process.env.CURSOR_TEST_EMAIL}))')
    const first = await probeCursorProviderStatus({ binaryPath, env: { ...process.env, CURSOR_TEST_EMAIL: "first@example.com" } })
    const second = await probeCursorProviderStatus({ binaryPath, env: { ...process.env, CURSOR_TEST_EMAIL: "second@example.com" } })
  ...
  it("rejects probe output that exceeds the shared byte budget", async () => {
    const binaryPath = await makeProbe("large", 'console.log(JSON.stringify({cliVersion:"2026.04.08",userEmail:"x".repeat(300*1024)})); setInterval(() => {}, 1000)')
    const result = await probeCursorProviderStatus({ binaryPath })
  ...
  it("retains failed probe cleanup and blocks another spawn until retry succeeds", async () => {
    ...
    const child = Object.assign(new EventEmitter(), {
      stdout: new PassThrough(), stderr: new PassThrough(), pid: 42,
      exitCode: null, signalCode: null,
    }) as unknown as childProcess.ChildProcess
    const nextChild = Object.assign(new EventEmitter(), {
      stdout: new PassThrough(), stderr: new PassThrough(), pid: 43,
      exitCode: 0, signalCode: null,
    }) as unknown as childProcess.ChildProcess
    const spawn = vi.mocked(childProcess.spawn).mockClear()
      .mockImplementationOnce(() => {
        queueMicrotask(() => child.stdout!.emit("data", Buffer.alloc(300 * 1024)))
        return child
      })
      .mockImplementationOnce(() => {
        queueMicrotask(() => {
          nextChild.stdout!.emit("data", JSON.stringify({ cliVersion: "2026.04.08", userEmail: "recovered@example.com" }))
          nextChild.emit("close", 0)
        })
        return nextChild
      })
    const cleanup = vi.spyOn(termination, "terminateProviderChildProcessTree")
    ...
    expect((await probeCursorProviderStatus({ binaryPath })).status).toBe("error")
    ...
    expect((await probeCursorProviderStatus({ binaryPath })).status).toBe("error")
    expect(spawn).toHaveBeenCalledTimes(1)
    expect((await probeCursorProviderStatus({ binaryPath })).auth.email).toBe("recovered@example.com")
    expect(spawn).toHaveBeenCalledTimes(2)
    expect(cleanup.mock.calls.map(([target]) => target)).toEqual([child, child, child])
  })

  it("accepts JSON surrounded by a UTF-8 BOM and whitespace", () => {
    const json = JSON.stringify({ cliVersion: "2026.04.08", userEmail: "user@example.com", subscriptionTier: "pro" })
    const parse = (stdout: string) => parseCursorAboutOutput({ stdout, stderr: "", code: 0 })
    expect(parse("\uFEFF \n" + json + "\n")).toEqual(parse(json))

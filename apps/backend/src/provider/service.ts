...
export class ProviderService {
  ...
  assertCanDispatch(threadId: string, kind: ProviderKind): void {
    ...
    if (!adapter) {
      throw new HttpError(404, `No adapter registered for ${kind}`, "provider_not_found");
    }
    ...
    if (this.maintenanceByThread.has(threadId) || this.activeTurns.has(threadId)) {
      throw new HttpError(
        409,
  ...
  dispatchTurnWithHandle(
    ...
    const sharedToken = options.sharedToken ?? this.threadTurnCoordinator?.reserveTurn(
      input.thread_id,
      `legacy:${kind}`,
    );
        ...
        if (
          ...
          const terminalEvent: ProviderRuntimeEvent = {
            ...
            payload: {
              ...
              provider_kind: kind,
              status: err instanceof ProviderTurnTimeoutError ? "timed_out" : "failed",
            },
          };
          await this.finalizeActiveTurn(
            input.thread_id,
   ...
   * Results are cached per-provider for {@link MODEL_CACHE_TTL_MS} (5 min).
   *

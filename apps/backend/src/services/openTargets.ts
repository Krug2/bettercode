...
import { buildWindowsCmdArgs, quoteWindowsCmdArg } from "../security/windowsCommandLine";
  ...
  for (const def of TARGETS) {
    ...
    for (const candidate of known) {
      try {
        if (
          await abortableDetection(
            deps.pathExists(candidate, signal),
            signal
          )
        ) {
          hit = candidate;
...
async function resolvedTargets(
  ...
  if (
    !opts.refresh &&
    state.cache &&
    now - state.cache.ts < CACHE_TTL_MS
  ) {
...
export async function launchOpenTarget(
  ...
  return await new Promise<{ ok: true }>((resolve, reject) => {
    ...
    const finish = (error?: Error) => {
      ...
      settled = true;
      clearTimeout(timer);
      if (error) reject(httpError(500, `failed to open target: ${error.message}`));
      else {

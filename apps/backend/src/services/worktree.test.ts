...
async function makeTestRepo(): Promise<string> {
  ...
  await exec("git", ["config", "user.email", "test@betterc0de.local"], { cwd: dir });
...
describe("shortThreadId", () => {
  it("strips hyphens and truncates to 8 chars", () => {
    expect(shortThreadId("12345678-abcd-1234-5678-abcdef012345")).toBe("12345678");
  });
...
describe("buildBranchName", () => {
  ...
  it("combines agent/<shortId>/<slug>", () => {
    expect(buildBranchName(threadId, "Fix login bug")).toBe("agent/12345678/fix-login-bug");
  });
...
describe("computeWorktreePath", () => {
  it.each(["../evil", "..\\evil", "a/b", "a\\b", "a:b", ".", "--", "CON", "LPT1"])(
    "keeps arbitrary thread ID %s inside its project directory",
    (threadId) => {
      const project = path.resolve("project");
      const expectedParent = path.dirname(computeWorktreePath(project, "safe-id"));
...
describe.skipIf(!gitAvailable)("WorktreeManager integration", () => {
  ...
  it.skipIf(!gitAvailable)(
    ...
    async () => {
      ...
      ).run(threadId, "proj-1", new Date().toISOString(), new Date().toISOString());
      ...
      const row = db
        .prepare(`SELECT branch, worktree_path, worktree_state FROM projection_threads WHERE thread_id = ?`)
        .get(threadId) as { branch: string; worktree_path: string; worktree_state: string };
  ...
  it.skipIf(!gitAvailable)(
    ...
    async () => {
      ...
      const row = db
        .prepare(`SELECT worktree_path, worktree_state FROM projection_threads WHERE thread_id = ?`)
        .get(threadId) as { worktree_path: string | null; worktree_state: string };
  ...
  it(
    "reconcile completes a persisted branch-deletion intent after physical removal",
    async () => {
      const repo = await makeTestRepo();
  ...
  it.skipIf(!gitAvailable)(
    ...
    async () => {
      ...
      fs.writeFileSync(path.join(created.worktreePath, "scratch.txt"), "remove me\n");
      ...
      expect(fs.readFileSync(path.join(created.worktreePath, "README.md"), "utf8")).toBe("seed\n");
      expect(fs.existsSync(path.join(created.worktreePath, "scratch.txt"))).toBe(false);
      expect(registry.findByThread(threadId)?.state).toBe("ready");

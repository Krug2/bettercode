...
describe("detectOpenTargets", () => {
  ...
  it("resolves via known install paths without a PATH lookup", async () => {
    ...
    const lookup = vi.fn(async (_names: string[]) => new Map<string, string>());
  ...
  it("exports the id sets the HTTP route validates against", async () => {
    ...
    expect([...SHELL_TARGET_IDS].sort()).toEqual(["git-bash", "terminal", "wsl"]);
  ...
  it("rate-limits explicit refreshes while returning the cached result", async () => {
    ...
    expect(
      rateLimited.find((target) => target.id === "zed")?.available
    ).toBe(true);
...
describe("wrapWindowsScriptInvocation", () => {
  ...
  it("wraps .bat through ComSpec with quoting and verbatim args", () => {
    const wrapped = wrapWindowsScriptInvocation(
      base("C:\\Users\\developer\\AppData\\Local\\GitHubDesktop\\bin\\github.bat")
    );
    ...
    expect(wrapped.args[3]).toContain('"C:\\Users\\developer\\My Projects\\app"');
...
describe("buildLaunchInvocation", () => {
  ...
  it("wsl prefers Windows Terminal when it is resolved", async () => {
    const inv = await buildLaunchInvocation(
      ...
      winDeps({
        pathExists: async (p) =>
          p.endsWith("wsl.exe") || p.endsWith("wt.exe"),
      })
    );
    expect(inv.command.endsWith("wt.exe")).toBe(true);

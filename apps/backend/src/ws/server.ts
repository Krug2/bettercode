...
export class WsHub {
  private readonly toolSnapshots = new ToolSnapshotBuffer({
    ...
    onError: (err) => logger.error({ err }, "failed to broadcast coalesced tool update"),
  });
  ...
  constructor(
    ...
    this.replayCapacity = Math.max(1, Math.floor(options.replayCapacity ?? 2_048));
    this.replayMaxBytes = Math.max(1, Math.floor(options.replayMaxBytes ?? 8 * 1024 * 1024));
    this.maxPayloadBytes = Math.max(1, Math.floor(options.maxPayloadBytes ?? 256 * 1024));
  ...
  attach(httpServer: HttpServer): void {
    ...
    this.upgradeHandler = (request, socket, head) => {
      ...
      const headerToken = typeof authHeader === "string"
        ...
        : null;
      const headerIsLocal = headerToken !== null
        && constantTimeEqual(headerToken, this.authToken);
      const cookieToken = this.options.sessionCookieName
        ? readCookie(request.headers.cookie, this.options.sessionCookieName)
        : null;
      if (!isAllowedBrowserOrigin(request.headers.origin, this.options.allowedOrigins, {
  ...
  private broadcastNow(frame: unknown): void {
    if (isProviderRuntimeFrame(frame)) {
      ...
      this.providerReplayJournal.push({ frame: sequencedFrame, serialized, bytes });
  ...
  private onConnection(
    ...
    if (preAuthenticatedPrincipal) {
      client = this.registerAuthenticatedClient(socket, preAuthenticatedPrincipal);
    }
    ...
    socket.on("message", async (raw) => {
      try {
        ...
        try {
          ...
          if (!authenticated) socket.close(WS_CLOSE_UNAUTHORIZED, "invalid frame");
          return;
        }
        ...
        if (typeof rpc.method !== "string" || typeof rpc.id !== "string" && typeof rpc.id !== "number") {
  ...
  private authenticateRemoteCredential(token: string): WsRemotePrincipal | null {
    try {
  ...
  private replayProviderEvents(
    ...
    const requestedAfter = !journalChanged
      ? request.afterSequence
      : 0;
    const earliestAvailable = this.providerReplayJournal[0]?.frame.sequence
      ?? this.providerSequence + 1;
    const retainedPrefixMissing = requestedAfter < earliestAvailable - 1;
    const unavailableFrameMissing = this.lastUnavailableProviderSequence > requestedAfter;
  ...
  private sendSerializedFrame(client: ReplayClient, serialized: string): boolean {
...
function isTailnetUpgrade(
  request: IncomingMessage,
  options: Pick<WsHubOptions, "tailnetSelfAddresses" | "trustProxyHeaders" | "trustLoopbackProxyHeaders">,
): boolean {
  if (
...
function upgradeProxyHeadersTrusted(
  ...
  return options.trustLoopbackProxyHeaders?.() === true
    && isLoopbackPeer(request.socket.remoteAddress);
}
...
function isProviderReplayRequest(value: unknown): value is ProviderReplayRequest {
  return (
    typeof value === "object"
    && value !== null
